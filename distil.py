...
# logger.setLevel(logging.CRITICAL) #不要print info
...
def convert_data_to_features(args, datas, tokenizer):
    ...
    for index, ele in enumerate(tqdm(datas)):
        try:
            ...
            input_tokens = ['[CLS]'] + context_tokens + ['[SEP]']
            token_type_ids = [0] * len(input_tokens)

            input_tokens += answer_tokens + ['[SEP]']
            ...
            for i, question_id in enumerate(question_ids):               
                ...
                SQ_input_ids.append(tokenizer.convert_tokens_to_ids('[MASK]'))
                ...
                if index < 2 :
                    ...
                    logger.info("tokens: %s" % " ".join(tokenizer.convert_ids_to_tokens(SQ_input_ids)))
...
def train(args, model, tea_model, tokenizer):

    """ Load datas """
    train_dataset = read_data(args.train_file)

    cached_train_features_file = args.train_file+'_cached'
    try:
    ...
    print("長度")
    len_=len(train_features)
    print(len_)
    len_ = int(len_ * args.data_rate)
    print("修改後")
    print(len_)
    train_features = train_features[:len_]
    print(len(train_features))
    ################
    # for i in range(100):
    #     print(tokenizer.convert_ids_to_tokens(train_features[i].labels))
    ################
    ...
    if os.path.isfile(os.path.join(args.tea_model, "optimizer.pt")) and os.path.isfile(
        os.path.join(args.tea_model, "scheduler.pt")
    ):
        # Load in optimizer and scheduler states
        optimizer.load_state_dict(torch.load(os.path.join(args.tea_model, "optimizer.pt")))
        scheduler.load_state_dict(torch.load(os.path.join(args.tea_model, "scheduler.pt")))
    ...
    if os.path.exists(args.tea_model):
        try:
            # set global_step to gobal_step of last saved checkpoint from model path
            checkpoint_suffix = args.tea_model.split("-")[-1].split("/")[0]
    ...
    for epoch, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # loss                      torch.Size([])                                                                    | 27/8957 [00:08<47:59,  3.10it/s]
            # tea loss                  torch.Size([])
            # logits                    torch.Size([6, 512, 21128])
            # tea logits                torch.Size([6, 512, 21128])
            # hidden_states             torch.Size([6, 512, 768])
            # tea hidden_states         torch.Size([6, 512, 768])
            # attentions                torch.Size([6, 12, 512, 512])
            # tea attentions            torch.Size([6, 12, 512, 512])
            ...
            # 蒸餾損失 
            with torch.no_grad():
                tea_outputs = tea_model(**inputs)

            loss += soft_cross_entropy(outputs.logits, tea_outputs.logits)

            if args.all_distil:
                for e in range(6):
                    loss += F.mse_loss(outputs.hidden_states[e], tea_outputs.hidden_states[e*2])
                    loss += F.mse_loss(outputs.attentions[e], tea_outputs.attentions[e*2])
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    args.tea_model, str(args.max_seq_length), str(args.doc_stride), str(args.max_answer_length), str(args.max_query_length))
    try:
...
def main():
    ...
    parser.add_argument("--stu_model", type=str, default="", help="stu_model name")
    parser.add_argument("--data_rate", type=float, default=1.0, help="data rate")
    parser.add_argument("--all_distil", type=bool, default=False, help="data rate")
    parser.add_argument("--dropout", type=float, help="stu_dropout")
    ...
    tea_config = AutoConfig.from_pretrained(
        args.config_name if args.config_name else args.tea_model,
        cache_dir=args.cache_dir if args.cache_dir else None,
    )
    #################
    tea_config.output_hidden_states = True
    tea_config.output_attentions = True
    #################
    # print(args.config_name, args.tea_model)
    # print(config)
    tokenizer = AutoTokenizer.from_pretrained(
        args.tokenizer_name if args.tokenizer_name else args.tea_model,
        do_lower_case=args.do_lower_case,
        cache_dir=args.cache_dir if args.cache_dir else None,
        use_fast=False,  # SquadDataset is not compatible with Fast tokenizers which have a smarter overflow handeling
    )
    tea_model = AutoModelForMaskedLM.from_pretrained(
        args.tea_model,
        from_tf=bool(".ckpt" in args.tea_model),
        config=tea_config,
        cache_dir=args.cache_dir if args.cache_dir else None,
    )
    stu_config = AutoConfig.from_pretrained(args.stu_model)
    #################
    stu_config.output_hidden_states = True
    stu_config.output_attentions = True
    stu_config.attention_probs_dropout_prob = args.dropout
    stu_config.hidden_dropout_prob = args.dropout
    #################
    model = AutoModelForMaskedLM.from_pretrained(args.stu_model, config=stu_config)
    # print(model)
    ...
    # Training
    global_step, tr_loss = train(args, model, tea_model, tokenizer)
